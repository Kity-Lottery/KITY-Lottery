...
import { explorerAddressUrl, formatToken, shortAddress } from "@/lib/format";
...
export function RoundCard({
  ...
  const { data: luckyTicket } = useReadContract({
    address: LOTTERY_ADDRESS,
    abi: lotteryAbi,
    functionName: "getTicket",
    args: [round?.luckyTicket ?? 0n],
    query: { enabled: round?.status === 3 },
  });
    ...
    <div className="card space-y-3">
      ...
      {isSettled && (
        <div className="space-y-2">
          ...
          {luckyTicket?.buyer && (
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/8 px-3 py-2 text-left text-xs text-amber-100/90">
              <div className="font-semibold text-amber-300">Lucky Wallet winner</div>
              <a
                href={explorerAddressUrl(chainId, luckyTicket.buyer as `0x${string}`)}
                target="_blank"
                rel="noreferrer noopener"
                className="mt-1 inline-flex items-center gap-1.5 font-mono text-amber-200 underline decoration-amber-300/50 underline-offset-2 hover:text-white"
              >
                {shortAddress(luckyTicket.buyer)}
                <span className="text-amber-300/70">BaseScan</span>
              </a>
            </div>
          )}
